...
struct slice{
    ...
    size_t length;    //number of indices included
...
struct Matrix_slice{
    /* 
    // TODO(Alex): Implement rest of definitions
     A Matrix_slice is a function object, and its operator()() does a stride calculation.
*/
    Matrix_slice()=default; //an empty matrix: no elements
    
    Matrix_slice(size_t offset, initializer_list<size_t> exts); //starting offset and extents
    Matrix_slice(size_t offset, initializer_list<size_t> exts, initializer_list<size_t> strs); // and strides
    ...
    size_t size:           //total number of elements in matrix
    size_t start;          //starting offset
    array<size_t,N> extends; //number of elements in each dimension
...
tempate<typename T, size_t N>
class Matrix_ref{
    /* 
    A Matrix_ref should not outlive its Matrix.
...
auto mr=user();  //trouble
     */
    public:
    // NOTE(Alex): Specific to Matrix_ref
...
template<typename U>
Matrix<T,N>& 
Matrix<T,N>::operator=(const Matrix_ref<U,N>& x){
